...
// Paginación: configurar el número de notas por página
$notasPorPagina = 5;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$offset = ($page - 1) * $notasPorPagina;

// Consultar la base de datos para obtener el total de notas
$total_query = "SELECT COUNT(*) as total FROM tb_tipos_notas";
$total_result = mysqli_query($conection, $total_query);
$total_row = mysqli_fetch_assoc($total_result);
$total_notas = $total_row['total'];

// Calcular el número total de páginas
$total_pages = ceil($total_notas / $notasPorPagina);

// Consultar las notas a mostrar en la página actual
$query = "SELECT * FROM tb_tipos_notas LIMIT $notasPorPagina OFFSET $offset";
...
<body>
    ...
    <div id="container">
        <div class="caja">
            ...
            <!-- Formulario de nueva nota (oculto por defecto) -->
            <div id="form-agregar" style="display:none;">
                <form action="insertar_tipo_nota.php" method="POST">
                    <input type="text" name="tipoNota" placeholder="Tipo de nota" required>
                    <button type="submit">Agregar</button>
                </form>
            </div>
            <p class="aviso"><i class="bi bi-info-circle"></i> Haz clic aquí <img src="../../assets/img/desplegar.png" alt="Icono"> para desplegar el formulario y registrar un nuevo ajuste contable.</p>
            <table>
                <thead>
                    <tr>
                        <th>Tipo de nota</th>
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['tipoNota']) . "</td>";
                        echo "<td>";
                        echo "<button class='btn-eliminar' data-id='" . $row['idTipoNota'] . "'>";
    ...
    <script>
       ...
       document.addEventListener('DOMContentLoaded', function() {
        ...
        if (message === 'added') {
            Swal.fire({
                title: '¡Nota agregada!',
                text: 'La nueva nota ha sido agregada con éxito.',
                icon: 'success',
    ...
    // Mostrar formulario de agregar nota
    ...
    button.addEventListener('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `eliminar_nota.php?id=${id}`;
            }
        })
    });
});
